...
*Setup instructions for a Gitian build of TES Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the triangleearth
...
- [Building TES Core](#building-triangleearth-core)
...
Clone the git repositories for triangleearth and Gitian.
...
in the triangleearth repository under 'Fetch and create inputs' to install sources which require
...
To build TES Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the triangleearth repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/triangleearth/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/triangleearth/triangleearth
...
For example:
```bash
URL=https://github.com/thrasher-/triangleearth.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit triangleearth=${COMMIT} --url triangleearth=${URL} ../triangleearth/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit triangleearth=${COMMIT} --url triangleearth=${URL} ../triangleearth/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit triangleearth=${COMMIT} --url triangleearth=${URL} ../triangleearth/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the triangleearth git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../triangleearth/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/triangleearth
SIGPATH=/some/root/path/triangleearth-detached-sigs

./bin/gbuild --url triangleearth=${BTCPATH},signature=${SIGPATH} ../triangleearth/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/triangleearth-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/triangleearth-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/triangleearth-osx-build.assert
```
...
[triangleearth/gitian.sigs.ltc](https://github.com/triangleearth/gitian.sigs.ltc/) repository, or if that's not possible create a pull
